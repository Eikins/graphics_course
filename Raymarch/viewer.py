...
class SimpleSquare:
    ...
    def draw(self, shader, mouse_pos, time, aspect_ratio, view_matrix):
        if(shader.glid):
            ...
            GL.glUniformMatrix4fv(GL.glGetUniformLocation(shader.glid,"_View"), 1, True, view_matrix)
    ...
    def run(self):
        ...
        while not glfw.window_should_close(self.win):
            ...
            self.square.draw(self.ray_tracer,self.mouse_pos+self.mouse_offset,
                             time.time()-begin,self.size[1]/self.size[0], self.trackball.view_matrix())
